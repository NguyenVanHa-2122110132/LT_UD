...
namespace MiniSupermarket.WinForms
{
    public partial class FormCategoryManagement : Form
    {
        ...
        private async Task LoadDataAsync()
        {
            try
            {
                // Gửi request GET tới endpoint "categories", tự động giải tuần tự hóa
                // chuỗi JSON thành List<CategoryDto>
        ...
        private async void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui lòng chọn nhóm hàng cần sửa!", "Cảnh báo",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private async void btnDelete_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(txtId.Text))
            {
                MessageBox.Show("Vui lòng chọn nhóm hàng cần xóa!", "Cảnh báo",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            int id = int.Parse(txtId.Text);
            var confirm = MessageBox.Show($"Bạn có chắc muốn xóa nhóm hàng ID = {id}?",
        ...
        private async void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var result = await _client.GetFromJsonAsync<List<CategoryDto>>(
